...
```{r mortal gdp expro for all excolonies, echo=FALSE}
...
```
```{r non_african effect of expropriation on gdp, echo=FALSE}
coef(mortalgdp)["settler_mortality"]/ coef(mortalexpro)["settler_mortality"]
...
```
```{r lm mortalgdp excolonies2, echo=FALSE}
...
```
```{r OLS regression of colonies, echo=FALSE}
#excolonies
...
ggplot(data=excolonies)+
  ...
  geom_abline(intercept = 9.341, 
              slope=-0.607, color="red")+theme(legend.position = "bottom",
        text=element_text(size=10))+labs(title="Figure 3: Relationship between settler mortality and Protection Against Expropriation",x="Settler Mortality", y="Protection against Expropriation",colour="Colonial Power")
